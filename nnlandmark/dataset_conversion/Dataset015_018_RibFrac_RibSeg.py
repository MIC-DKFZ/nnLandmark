...
if __name__ == '__main__':
    ...
    imagesTr = join(nnLM_raw, target_dataset_name, 'imagesTr')
    imagesTs = join(nnLM_raw, target_dataset_name, 'imagesTs')
    labelsTr = join(nnLM_raw, target_dataset_name, 'labelsTr')
    ...
    generate_dataset_json(
        join(nnLM_raw, target_dataset_name),
        channel_names={0: 'CT'},
        labels = {
            'background': 0,
            'fracture': (1, 2, 3, 4, 5),
    ...
    for c in identifiers:
        if c in skip_identifiers:
            continue
        print(c)
        tr_file = join('$nnLM_raw', target_dataset_name_ribfrac, 'imagesTr', c + '_0000.nii.gz')
        ts_file = join('$nnLM_raw', target_dataset_name_ribfrac, 'imagesTs', c + '_0000.nii.gz')
