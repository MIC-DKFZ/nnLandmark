...
if __name__ == '__main__':
    ...
    target_dataset_dir = join(nnLM_raw, target_dataset)
    maybe_mkdir_p(target_dataset_dir)

    dataset = get_filenames_of_train_images_and_targets(join(nnLM_raw, source_dataset))
    ...
    # nnLM_raw is in different locations

    # paths must be relative to target_dataset_dir!!!
    for k in dataset.keys():
        dataset[k]['label'] = join('$nnLM_raw', os.path.relpath(dataset[k]['label'], nnLM_raw))
        dataset[k]['images'] = [join('$nnLM_raw', os.path.relpath(i,  nnLM_raw)) for i in dataset[k]['images']]

    # load old dataset.json
    dataset_json = load_json(join(nnLM_raw, source_dataset, 'dataset.json'))
