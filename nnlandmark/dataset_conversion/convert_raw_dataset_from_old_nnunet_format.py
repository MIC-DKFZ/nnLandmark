...
def convert(source_folder, target_dataset_name):
    ...
    source_folder
    """
    if isdir(join(nnLM_raw, target_dataset_name)):
        raise RuntimeError(f'Target dataset name {target_dataset_name} already exists. Aborting... '
                           ...
                           f'delete {join(nnLM_raw, target_dataset_name)}')
    maybe_mkdir_p(join(nnLM_raw, target_dataset_name))
    shutil.copytree(join(source_folder, 'imagesTr'), join(nnLM_raw, target_dataset_name, 'imagesTr'))
    shutil.copytree(join(source_folder, 'labelsTr'), join(nnLM_raw, target_dataset_name, 'labelsTr'))
    if isdir(join(source_folder, 'imagesTs')):
        shutil.copytree(join(source_folder, 'imagesTs'), join(nnLM_raw, target_dataset_name, 'imagesTs'))
    if isdir(join(source_folder, 'labelsTs')):
        shutil.copytree(join(source_folder, 'labelsTs'), join(nnLM_raw, target_dataset_name, 'labelsTs'))
    if isdir(join(source_folder, 'imagesVal')):
        shutil.copytree(join(source_folder, 'imagesVal'), join(nnLM_raw, target_dataset_name, 'imagesVal'))
    if isdir(join(source_folder, 'labelsVal')):
        shutil.copytree(join(source_folder, 'labelsVal'), join(nnLM_raw, target_dataset_name, 'labelsVal'))
    shutil.copy(join(source_folder, 'dataset.json'), join(nnLM_raw, target_dataset_name))

    dataset_json = load_json(join(nnLM_raw, target_dataset_name, 'dataset.json'))
    ...
    save_json(dataset_json, join(nnLM_raw, target_dataset_name, 'dataset.json'), sort_keys=False)
