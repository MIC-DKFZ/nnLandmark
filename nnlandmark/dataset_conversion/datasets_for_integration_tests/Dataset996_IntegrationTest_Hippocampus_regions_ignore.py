...
if __name__ == '__main__':
    ...
    try:
        ...
        if existing_dataset_name != dataset_name:
            raise FileExistsError(f"A different dataset with id {dataset_id} already exists :-(: {existing_dataset_name}. If "
                               f"you intent to delete it, remember to also remove it in nnLM_preprocessed and "
                               f"nnLM_results!")
    except RuntimeError:
        pass

    if isdir(join(nnLM_raw, dataset_name)):
        shutil.rmtree(join(nnLM_raw, dataset_name))

    source_dataset = maybe_convert_to_dataset_name(4)
    shutil.copytree(join(nnLM_raw, source_dataset), join(nnLM_raw, dataset_name))

    # additionally optimize entire hippocampus region, remove Posterior
    dj = load_json(join(nnLM_raw, dataset_name, 'dataset.json'))
    dj['labels'] = {
        'background': 0,
        'hippocampus': (1, 2),
        'anterior': 1,
    ...
    save_json(dj, join(nnLM_raw, dataset_name, 'dataset.json'), sort_keys=False)
    ...
    segs = nifti_files(join(nnLM_raw, dataset_name, 'labelsTr'))
    for s in segs:
