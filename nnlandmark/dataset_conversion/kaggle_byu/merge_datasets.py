...
import os
del os.environ['nnLM_preprocessed']
del os.environ['nnLM_results']
...
if __name__ == '__main__':
    current_nnunet_raw = os.environ['nnLM_raw']
    ...
    out_base = join(nnLM_raw, target_datset_name)
    ...
    for sd in source_datasets:
        raw_dir = join(nnLM_raw, maybe_convert_to_dataset_name(sd))
        ...
        for k, v in ds.items():
            dataset[k] = {
                'images': [i.replace(current_nnunet_raw, '$nnLM_raw') for i in ds[k]['images']],
                'label': ds[k]['label'].replace(current_nnunet_raw, '$nnLM_raw')
            }
        targets.update(lb)
    ...
    source_dsjs = [load_json(join(nnLM_raw, maybe_convert_to_dataset_name(i), 'dataset.json')) for i in source_datasets]
