...
if __name__ == '__main__':
    ...
    for d in ds:
        tc = load_json(join(nnLM_raw, d, 'train_coordinates.json'))
        tcos = load_json(join(nnLM_raw, d, 'train_coordinates_forOrigShapes.json'))

        tc.update(source_train_coords)
        tcos.update(source_train_coords_origshape)
        save_json(tc, join(nnLM_raw, d, 'train_coordinates.json'))
        save_json(tcos, join(nnLM_raw, d, 'train_coordinates_forOrigShapes.json'))
