...
def evaluate_measurements(predictions_path: Path, ground_truth_path: Path, spacing_path: Path, output_path: Path):
    ...
    Args:
        ...
        ground_truth_path: Path to the ground truth JSON (in voxel space) in nnLM_raw.
        spacing_path: Path to the spacing JSON in nnLM_raw.
...
def biometry_entry_point():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default="/path/to/evaluation/Dataset742_FeTA_2_4/nnLandmark_trainer/prediction/measurement.json"
    )
