...
def find_candidate_datasets(dataset_id: int):
    ...
    if nnLM_preprocessed is not None and isdir(nnLM_preprocessed):
        candidates_preprocessed = subdirs(nnLM_preprocessed, prefix=startswith, join=False)
    else:
        candidates_preprocessed = []

    if nnLM_raw is not None and isdir(nnLM_raw):
        candidates_raw = subdirs(nnLM_raw, prefix=startswith, join=False)
    else:
        candidates_raw = []

    candidates_trained_models = []
    if nnLM_results is not None and isdir(nnLM_results):
        candidates_trained_models += subdirs(nnLM_results, prefix=startswith, join=False)
...
def convert_id_to_dataset_name(dataset_id: int):
    ...
    if len(unique_candidates) > 1:
        raise RuntimeError("More than one dataset name found for dataset id %d. Please correct that. (I looked in the "
                           "following folders:\n%s\n%s\n%s" % (dataset_id, nnLM_raw, nnLM_preprocessed, nnLM_results))
    if len(unique_candidates) == 0:
        raise RuntimeError(f"Could not find a dataset with the ID {dataset_id}. Make sure the requested dataset ID "
                           ...
                           f"nnLM_preprocessed={os.environ.get('nnLM_preprocessed') if os.environ.get('nnLM_preprocessed') is not None else 'None'}\n"
                           f"nnLM_results={os.environ.get('nnLM_results') if os.environ.get('nnLM_results') is not None else 'None'}\n"
                           f"nnLM_raw={os.environ.get('nnLM_raw') if os.environ.get('nnLM_raw') is not None else 'None'}\n"
